...
<head>
    ...
    <link rel="stylesheet" href="home_style2.css" media="all"/>
</head>
<style>
#scroll_messages{
	max-height: 500px;
...
<?php
		//getting user to id and user from id for messages
			...
			$get_user = "select * from users where users_id='$get_id'";

			$run_user = mysqli_query($con,$get_user);

			$row_user=mysqli_fetch_array($run_user);

			$user_to_msg = $row_user['users_id'];
	...
	<div class="col-sm-6">
		<div class="load_msg" id="scroll_messages">
			<?php
				...
				$sel_msg = "select * from users_messages where (user_to='$user_to_msg' AND user_from='$user_from_msg') OR (user_from='$user_to_msg' AND user_to='$user_from_msg') ORDER by 1 ASC;";
                
                $run_msg = $con->query($sel_msg);
				while($row_msg=$run_msg->fetch_assoc()){
		...
		<?php
			if(isset($_POST['send_msg'])){
				...
				$insert = "insert into users_messages(user_to,user_from,msg_body,date,msg_seen) values ('$user_to_msg','$user_from_msg','$msg',NOW(),'no')";
		...
		<?php
			...
			echo "
				<div class='row'>
					...
					<img class='img-circle' src='$user_image' width='150' height='150' />
					<br><br>
					<ul class='list-group'>
